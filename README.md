...
> **Development integration, not final qualification:** v0.2.0 remains the immutable, source-only Core release; it does not contain Binder or these new public examples. The [Core positive example](#bounded-positive-core-example) and [Binder fake companion](#binder-companion-controlled-fake-example) are implemented and have retained Windows development-run evidence. A real Hermes Request-first run is now recorded for implementation commit `d4f9f4d8` in [the dogfood record](docs/HERMES-BINDER-DOGFOOD-d4f9f4d8.md): its first two failures remain retained and its third attempt reached a **rejected** Admission. It is not final-tree qualification. The recipes below still need final-tree shell execution and final platform qualification; see the [pending ledger](#pending-command-and-example-integration). No new release or qualified Binder commit is claimed.
...
| Initial Hermes adapter | Binder's literal-argv process route can invoke an approved configured executable; Hermes is the first intended real-agent integration | A real Windows Request-first run reached Store Result and rejected Admission; see the [dogfood record](docs/HERMES-BINDER-DOGFOOD-d4f9f4d8.md). It does not qualify other adapters, platforms, or reviewer correctness; Core requires neither. |
...
**Real Hermes observed, final qualification pending:** [a Windows Request-first run](docs/HERMES-BINDER-DOGFOOD-d4f9f4d8.md) used the actual installed Hermes executable through the Binder, retained two capture-completion failures and a third Result/Admission path, and cold-inspected all three attempts. The third admission was rejected. Do not replace the fake command with guessed Hermes flags, treat a copied historical log as a new review, or treat that rejection as a reviewer-correctness verdict. The final fixed-revision install recipe and exact-final-tree command ledger are still required; no Hermes output is supplied here.
...
| Binder real review | A real Windows Hermes run is recorded at `d4f9f4d8` in [the dogfood record](docs/HERMES-BINDER-DOGFOOD-d4f9f4d8.md): two retained capture failures followed by a retained Result and rejected Admission with cold inspection. Re-execute a new Request-first run only against the exact final tree; fake subprocess fixtures do not substitute. |
