...
3. **Check real prerequisites.** Confirm OS/shell, installed Rust toolchain, components and linker. Keep `Cargo.lock` and use locked builds. Core requires no Hermes. The [Bash quick-start setup](#quick-start-setup--bash) uses installed Rust/Cargo 1.97.1, native absolute paths and `sha256sum` or `shasum` for binary identity. Before the Hermes adapter is used, separately obtain approval for the existing agent configuration, network/provider use, budget and runtime bounds; do not install or launch an agent merely because this section mentions one. The fake companion recipe is available above; real-Hermes arguments must be explicitly supplied for a new run. Native implementation qualification is complete; verify the annotated tag and matching GitHub Release as the publication identity.
