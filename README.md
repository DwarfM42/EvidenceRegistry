...
> **Development integration, not final qualification:** v0.2.0 remains the immutable, source-only Core release; it does not contain Binder or these new public examples. The [Core positive example](#bounded-positive-core-example) and [Binder fake companion](#binder-companion-controlled-fake-example) are implemented and have retained Windows development-run evidence. A real Hermes Request-first run against implementation commit `98b7646` produced a retained Result and **accepted** Admission; its retained preceding launcher/process failures and scope limits are in [the dogfood record](docs/HERMES-BINDER-DOGFOOD-98b7646.md). It is not final-documentation-tree or multi-platform qualification. The recipes below still need final-tree shell execution and final platform qualification; see the [README verification ledger](#readme-verification-ledger-and-remaining-final-execution). No new release or qualified Binder commit is claimed.
...
| Initial Hermes adapter | Binder's literal-argv process route can invoke an approved configured executable; Hermes is the first intended real-agent integration | A real Windows Request-first run reached Store Result and accepted Admission on implementation commit `98b7646`; see the [dogfood record](docs/HERMES-BINDER-DOGFOOD-98b7646.md). It does not qualify other adapters, platforms, reviewer correctness, or independent-review quorum; Core requires neither. |
...
**Real Hermes observed, final qualification pending:** [a Windows Request-first run](docs/HERMES-BINDER-DOGFOOD-98b7646.md) used the actual installed Hermes executable through the Binder, retained isolated launcher/process failures separately, then completed a fresh Request-first Result/accepted-Admission path with cold inspection. Do not replace the fake command with guessed Hermes flags, treat a copied historical log as a new review, or treat that acceptance as a reviewer-correctness or independence verdict. The final fixed-revision install recipe and exact-final-documentation-tree command ledger are still required; no Hermes output is supplied here.
...
| Real Hermes review | One Windows/Git-Bash Request-first Hermes run on implementation commit `98b7646` retained a Result and accepted selected-lane Admission; its exact boundary is in [the dogfood record](docs/HERMES-BINDER-DOGFOOD-98b7646.md). | Report only the OS/adapter actually exercised, with Request-to-cold-replay evidence and all attempts; this does not qualify the final documentation tree, semantic correctness, reviewer identity, independence, or other platforms/adapters. |
...
| Binder real review | A real Windows Hermes run is recorded at `98b7646` in [the dogfood record](docs/HERMES-BINDER-DOGFOOD-98b7646.md): isolated path/launcher failures are retained separately; a fresh Request-first run produced a retained Result and accepted Admission with cold inspection. Re-execute a new Request-first run only against the exact final documentation tree; fake subprocess fixtures do not substitute. |
