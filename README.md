...
> **Development integration, not final qualification:** v0.2.0 remains the immutable, source-only Core release; it does not contain Binder or these new public examples. The [Core positive example](#bounded-positive-core-example) and [Binder fake companion](#binder-companion-controlled-fake-example) are implemented and have retained Windows development-run evidence. A fresh real Hermes Request-first run against commit `0a44589` produced a retained Result and **accepted** Admission; its limits and retained earlier failures are in [the current dogfood record](docs/HERMES-BINDER-DOGFOOD-0a44589.md). It is not final-documentation-tree or multi-platform qualification. The recipes below still need final-tree shell execution and final platform qualification; see the [README verification ledger](#readme-verification-ledger-and-remaining-final-execution). No new release or qualified Binder commit is claimed.
...
| Initial Hermes adapter | Binder's literal-argv process route can invoke an approved configured executable; Hermes is the first intended real-agent integration | A real Windows Request-first run reached Store Result and accepted Admission on commit `0a44589`; see the [dogfood record](docs/HERMES-BINDER-DOGFOOD-0a44589.md). It does not qualify other adapters, platforms, reviewer correctness, or independent-review quorum; Core requires neither. |
...
| Real Hermes review | One Windows/Git-Bash Request-first Hermes run on commit `0a44589` retained a Result and accepted selected-lane Admission; its exact boundary is in [the dogfood record](docs/HERMES-BINDER-DOGFOOD-0a44589.md). | Report only the OS/adapter actually exercised, with Request-to-cold-replay evidence and all attempts; this does not qualify the final documentation tree, semantic correctness, reviewer identity, independence, or other platforms/adapters. |
